...
void draw_carros(ALLEGRO_BITMAP *carro, ALLEGRO_DISPLAY *display, Car *vet_carros){
	...
	if((vet_carros-> largura_carro) < LARGURA_PISTA_INF || (vet_carros -> altura_carro) < ALTURA_TELA){		
		carro = al_create_bitmap(regra_tres_largura_carro(vet_carros -> largura_carro), regra_tres_altura_carro(vet_carros -> altura_carro));	// Largura e altura do carro
		al_set_target_bitmap(carro);
		...
		//
		//vet_carros -> x += 5.5;
		vet_carros -> largura_carro+= 4.0;
		vet_carros -> altura_carro+= 4.2;
		vet_carros -> y += 2.5	;
	}
}
...
void draw_rodas(ALLEGRO_BITMAP *roda, ALLEGRO_DISPLAY *display, Car *vet_carros){
	 	...
	 	roda = al_create_bitmap(regra_tres_largura_roda(vet_carros -> largura_roda), regra_tres_altura_roda(vet_carros -> altura_roda));
	 	vet_carros -> roda2_x = vet_carros -> x + (regra_tres_largura_carro(vet_carros -> larg_sup) * 2.2);
	 	al_set_target_bitmap(roda);
	 	al_clear_to_color(al_map_rgb(0,0,0));
	 	al_set_target_bitmap(al_get_backbuffer(display));
	 	...
	 	vet_carros -> largura_roda += 5;   // Variação da largura da roda
	 	vet_carros -> altura_roda += 4;	   // Variação da altura da roda
	 	vet_carros -> roda1_y += 2.5;     // Velocidade das rodas
	 	vet_carros -> roda2_y +=2.5;
	 	vet_carros -> larg_sup += 1.60;    // Variação de 50 até 833
...
int main(int argc, char **argv) {
	...
	// Rodas
	meu_carro.largura_roda = LARGURA_RODA;
	meu_carro.altura_roda = ALTURA_RODA;
	...
	// Configurando a variação do meu carro
	 float delta_dx = LARGURA_TELA / 200.0;      // Variando 5 pixels cada tecla apertada
	...
	for(unsigned i = 0; i < NUMCARS; i++) {
		...
		vet_carros[i].largura_roda = LARGURA_RODA;
		vet_carros[i].altura_roda = ALTURA_RODA;
		vet_carros[i].larg_sup = LARGURA_PISTA_SUP;      // Para variar ao longo da descida 50 até 833
		// Coordenadas
		vet_carros[i].x = (475 + rand() % 50);
		vet_carros[i].y = ALTURA_CEU;
		vet_carros[i].roda1_x = vet_carros[i].x;
		vet_carros[i].roda1_y = vet_carros[i].y - 3;
		vet_carros[i].roda2_x = vet_carros[i].x;
		vet_carros[i].roda2_y = vet_carros[i].y - 3;
		//
		vet_carros[i].cor = cor_aleatoria[i];
		vet_carros[i].id = i + 1;
	...
	while(playing == 1) {
		...
		// Se o tipo de evento for um temporizador, ou seja, se o tempo passou de t para t + 1
			...
			//for(unsigned i = 0; i < NUMCARS; i++){
				...
				draw_rodas(roda, display, &vet_carros[0]);
			//}
